...
public abstract class Weapon extends Item {
	...
	public Weapon(String name, int x, int y, int ammo) {
		...
		this.ammo = ammo;
		this.weaponImage[0] = new Image(
				ClassLoader.getSystemResource("weapons/" + name + "/" + name + "_down.png").toString());
		this.weaponImage[1] = new Image(
				ClassLoader.getSystemResource("weapons/" + name + "/" + name + "_diagdown.png").toString());
		this.weaponImage[2] = new Image(
				ClassLoader.getSystemResource("weapons/" + name + "/" + name + "_side.png").toString());
		this.weaponImage[3] = new Image(
				ClassLoader.getSystemResource("weapons/" + name + "/" + name + "_diagup.png").toString());
		this.weaponImage[4] = new Image(
				ClassLoader.getSystemResource("weapons/" + name + "/" + name + "_up.png").toString());
	}
	...
	public void render(GraphicsContext gc, int facingDirection, int mirrorDirection, boolean renderFirst) {
		...
		gc.drawImage(weaponImage[facingDirection], 0, 0, imgWidth, imgHeight,
				MainApplication.SCREEN_WIDTH / 2 - imgWidth / 2 * mirrorDirection * MainApplication.SIZE_MULTIPLIER,
